    ...
    const eliminarPerfil = async (id) => {
        ...
        if (confirmacion) {
            ...
            try {
                ...
                //Elimina los datos del jugador en firestore
                const jugador = await getItemsPL(email);
    ...
    const saveChanges = async () => {
        try {
            if (lista.length > 0) {
                ...
                await updateDoc(doc(db, 'jugadores', usuario), editedData);
                setEditMode(false);
            } else {
    ...
    useEffect(() => {
        onAuthStateChanged(auth, async (usuarioFirebase) => {
            if (usuarioFirebase) {
                ...
                setUsuario(jugadorA.id);
                setEmail(usuarioFirebase.email);
                setEquipo(jugadorA.equipo);
                if (jugadorA.id) {
                    setUsuario(jugadorA.id)
                    const docSnapshot = await getDoc(doc(db, 'jugadores', jugadorA.id));
                            ...
                            <div className='vertical-table-container'>
                                {lista.map(list => (
                                    <div key={list.id} className='vertical-row'>
                                        ...
                                        <div>
                                            Estatura: {editMode ? (
                                                <input
                                                    ...
                                                    onChange={(e) => handleEdit("estatura", e.target.value.toString())} // Convertir a String utilizando toString()
                                                />
                                            ) : (
                                                list.estatura + " cm"
                                            )}
                                        ...
                                        <div>
                                            Peso: {editMode ? (
                                                <input
                                                    ...
                                                    onChange={(e) => handleEdit("peso", e.target.value.toString())} // Convertir a String utilizando toString()
                                                />
                                            ) : (
                                                list.peso + " kg"
                                            )}
                                        ...
                                        <div>
                                            Posición: {editMode ? (
                                                <select value={editedData.posicion} onChange={(e) => handleEdit("posicion", e.target.value)}>
